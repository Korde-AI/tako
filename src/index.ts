...
async function runStart(): Promise<void> {
  ...
  const manageDiscordProjectMemberFromTool = async (
    ...
    const resolved = discordProjectSupport.resolvePrincipalIdentity(targetIdentity)
      ?? await discordProjectSupport.resolvePrincipalIdentityFromRoom(targetIdentity, ctx);
    if (!resolved) {
      return {
  ...
  const inspectCurrentDiscordProjectRoom = async (
    ctx: import('./tools/tool.js').ToolContext,
  ) => {
    const executionContext = ctx.executionContext;
  ...
  const discordProjectSupport = createDiscordProjectSupport({
    principalRegistry,
    normalizeIdentity: normalizeDiscordPolicyIdentity,
